...
export function ChaptersSection({ bookId, t }: ChaptersSectionProps) {
  ...
  return (
    <SidebarCard title={t("chaptersSection.title")}>
      {chapters.length === 0 ? (
        <p className="text-xs text-muted-foreground/50 italic">
          {t("chaptersSection.empty")}
        </p>
      ) : (
            ...
            return (
              <li
                ...
                <span className="truncate flex-1">
                  {String(ch.number).padStart(2, "0")} {ch.title || t("chapter.label", { n: ch.number })}
                </span>
